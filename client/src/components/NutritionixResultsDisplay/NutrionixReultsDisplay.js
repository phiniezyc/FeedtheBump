                ...
                {(hits || []).map((result, i) => (
                        <div key={i} className='nutritionixResultDiv'>
                            <Card className='cardDisplay' header={<CardTitle reveal  image='' waves='light'/>}
                                title={result.fields.item_name}reveal={<div><p>{`Calories: ${result.fields.nf_calories}`}</p><p>{`Protein: ${result.fields.nf_protein}`}</p><p>{`Calcium: ${result.fields.nf_calcium_dv}`}</p><p>{`Iron: ${result.fields.nf_iron_dv}`}</p></div>}> <p><a href="/">This is a link</a></p>
